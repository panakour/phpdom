...
class DomTest extends TestCase
{
    ...
    public function testExceptionOnReplaceWrongAmountOfAttributesOnElement()
    {
        ...
        $this->expectExceptionMessage(
            sprintf(
                "Items are not equal. The number of original elements are: %d but you give %d",
                $this->dom->getNodeList('img')->length,
                count($wrongAmountOfAttributes)
            )
        );
    ...
    public function testRemoveElementByDomElement()
    {
        $elementToRemove = $this->dom->DOMDocument->getElementById('example-image-first');
        $this->dom->removeElementByDomElement($elementToRemove);
        $this->assertStringNotContainsString('example-image-first', $this->dom->get());
    }

    public function testReplaceImagesAttributes()
    {
        $name = 'testImage';
        $storagePath = 'http://example.com/images/';
        $class = 'custom-class';
        $this->dom->replaceImagesAttributes($name, $storagePath, $class);

        $srcValues = $this->dom->getAttributesValue('img', 'src');
        $altValues = $this->dom->getAttributesValue('img', 'alt');
        $classValues = $this->dom->getAttributesValue('img', 'class');

        foreach ($srcValues as $src) {
            $this->assertEquals($storagePath . $name, $src);
        }

        foreach ($altValues as $alt) {
            $this->assertEquals($name, $alt);
        }

        foreach ($classValues as $classValue) {
            $this->assertEquals($class, $classValue);
        }
    }

    public function testReplaceImagesSourceToAbsolute()
    {
        $baseURL = 'http://www.example.com/';
        $this->dom->replaceImagesSourceToAbsolute($baseURL);
        $sourceAttributesValue = $this->dom->getAttributesValue('img', 'src');
        foreach ($sourceAttributesValue as $src) {
            $this->assertFalse(ToolBox::isRelativeUrl($src));
        }
    }
